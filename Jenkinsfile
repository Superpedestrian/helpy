...
pipeline {
  ...
  stages {
    ...
    stage('Run Pods') {
        agent { label 'EKS-Node' }
        steps {
            sh 'kubectl run $branch_ns --namespace=$branch_ns --image=$docker_image:$branch_ns --image-pull-policy=Always --port=8088 --env="DATABASE_URL=postgis://postgres@postgres:5432/postgres"'
        }
    }
    stage('Test') {
        agent { label 'EKS-Node' }
